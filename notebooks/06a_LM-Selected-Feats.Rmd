...
bind_ob_uniq %>% 
  add_count(feature) %>% 
  arrange(quick_assoc, -n, feature, metadata)
```

```{r}
# Unique to CHS adj by each obesity measure
bind_chs_uniq = bind_rows(mutate(uniq_bmiCHS_mod_annot, adj = "bmi"), 
                   mutate(uniq_wcCHS_mod_annot, adj = "wc"),
                   mutate(uniq_bfCHS_mod_annot, adj = "bf")) 
...
bind_core = bind_rows(mutate(BMI_CHS_mod_annot, adj = "bmi"), 
                   mutate(WC_CHS_mod_annot, adj = "wc"),
                   mutate(BF_CHS_mod_annot, adj = "bf")) %>% 
...
```{r}
bind_ob_uniq %>% 
  write_tsv(., file.path(tab_dir, "ob_uniq_mod_feats.tsv"))
bind_chs_uniq %>% 
  write_tsv(., file.path(tab_dir, "chs_uniq_mod_feats.tsv"))
bind_core %>% 
  write_tsv(., file.path(tab_dir, "core_mod_feats.tsv"))
```
...
## Write Ortholog tables
```{r}
bind_ob_uniq_ortho %>% 
  write_tsv(., file.path(tab_dir, "ob_uniq_ortho_feats.tsv"))
bind_chs_uniq_ortho %>% 
  write_tsv(., file.path(tab_dir, "chs_uniq_ortho_feats.tsv"))
bind_core_ortho %>% 
  write_tsv(., file.path(tab_dir, "core_ortho_feats.tsv"))

# Did file write?
file.exists(file.path(tab_dir, "ob_uniq_ortho_feats.tsv"))
```